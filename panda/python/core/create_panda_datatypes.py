...
def main():
    '''
    copy all the header files from panda build directory to known places in the python project. Rewrite as we go/as needed
    produce panda_datatypes.py which references all of them
    '''
    ...
    # Build PandaCB namedtuple by reading panda's cb-defs.h and finding all PANDA_CB_[stuff] to put in cb_list (order matters)
    cb_list = {-1: "init"} # XXX: Is init important? Luke thinks it might be. -1 bc it doesn't match any real panda cbs
    cbn = 0
    with open (os.path.join(INCLUDE_DIR_PAN, "callbacks/cb-defs.h")) as fp:
        for line in fp:
            match = re.search("^(\s+)PANDA_CB_([^,]+)\,", line)
            if match:
                cbname = match.groups()[1]
                cbname_l = cbname.lower()
                cb_list[cbn] = cbname_l
                cbn += 1

    # Look through the typedefs at the end of cb-defs.h to get type information for each callback for cb_types
    in_tdu = False # TypeDef-Union
    cb_types = {-1: ("init", "bool(void*)")} #-1 for init?
    with open (os.path.join(INCLUDE_DIR_PAN, "callbacks/cb-defs.h")) as fp:
        for line in fp:
            if re.search("typedef union panda_cb {", line): # Typedef start
                in_tdu = True
                continue
            if re.search("} panda_cb;", line): # Typedef end
                in_tdu = False
                continue
            if in_tdu:
                ...
                for i in range(cbn):
                    ...
                    if match:
                        ...
                        partypes = []
                        for param in params.split(','): # For each argument return its type
                            # Find rightmost non-alpha-numeric_ character
                            match = re.search(r'[^(a-zA-Z0-9_)]', param[::-1])
                            if match:
                                typ = param[:len(param)-match.start()] # Found a match, right trim
                            else:
                                typ = param
                            partypes.append(typ)
    ...
    # Build up callback dict strings going from like pcb.before_block_translate -> pandacbtype("before_block_translate", C.PANDA_CB_BEFORE_BLOCK_TRANSLATE)
    callback_dict_mappings = {}
    for i in range(cbn):
        cb_name, cb_type = cb_types[i]
        callback_dict_mappings[f'pcb.{cb_name}'] =  f'pandacbtype("{cb_name}", C.PANDA_CB_{cb_name.upper()})'

    # defined here to avoid prohibited use of \n in f-strings
    delim = ", \n"
...
def define_clean_header(ffi, fname):
    ...
    # CFFI can't handle externs, but sometimes we have to extern C (as opposed to something?)
    r = open(fname).read()
    for line in r.split("\\n"):
...
inc_dir_pyp = "{INCLUDE_DIR_PYP}"

# PPP Headers
# Syscalls - load architecture-specific headers
if arch == "i386":
        define_clean_header(ffi, inc_dir_pyp+"/panda_datatypes_X86_32.h")
        define_clean_header(ffi, inc_dir_pyp+"/syscalls_ext_typedefs_x86.h")
elif arch == "x86_64":
        define_clean_header(ffi, inc_dir_pyp+"/panda_datatypes_X86_64.h")
        define_clean_header(ffi, inc_dir_pyp+"/syscalls_ext_typedefs_x64.h")
elif arch == "arm":
        define_clean_header(ffi, inc_dir_pyp+"/panda_datatypes_ARM_32.h")
        define_clean_header(ffi, inc_dir_pyp+"/syscalls_ext_typedefs_arm.h")
elif arch == "ppc" and int(bits) == 32:
    define_clean_header(ffi, inc_dir_pyp+"/panda_datatypes_PPC_32.h")
    print('WARNING: no syscalls support for PPC 32')
elif arch == "ppc" and int(bits) == 64:
    define_clean_header(ffi, inc_dir_pyp+"/panda_datatypes_PPC_64.h")
    print('WARNING: no syscalls support for PPC 64')
elif arch == "mips" and int(bits) == 32:
        ffi.cdef(read_cleanup_header(inc_dir_pyp+"/panda_datatypes_MIPS_32.h"))
elif arch == "mipsel" and int(bits) == 32:
        ffi.cdef(read_cleanup_header(inc_dir_pyp+"/panda_datatypes_MIPS_32.h")) # XXX?
else:
        print("PANDA_DATATYPES: Architecture not supported")

# Define some common panda datatypes
#define_clean_header(ffi, inc_dir_pyp+"/panda_qemu_support.h")
define_clean_header(ffi, inc_dir_pyp+"/panda_datatypes.h")

# Now syscalls2 common:
define_clean_header(ffi, inc_dir_pyp+"/syscalls2_info.h")
...
define_clean_header(ffi, inc_dir_pyp+"/syscalls_ext_typedefs.h")

define_clean_header(ffi, inc_dir_pyp+"/callstack_instr.h")
# END PPP headers

define_clean_header(ffi, inc_dir_pyp+"/breakpoints.h")

# Include each header file we parsed during generation
...
# Build namedtuple for panda callback names
PandaCB = namedtuple("PandaCB", [{delim.join([repr(cb_list[k]) for k in sorted(cb_list)])}])

# Types of each callback TODO sort numerically?
pcb = PandaCB({delim.join([cb_types[k][0] + ' = ffi.callback("' + cb_types[k][1] + '")' for k in sorted(cb_types)])})

pandacbtype = namedtuple("pandacbtype", "name number")

# Map each pcb.callback to a pandacbtype
callback_dictionary = {{ {delim.join([(k + ": " + v) for (k,v) in callback_dict_mappings.items()])} }}
...
""")
        ...
        # Parse a few files and add panda-filtered contents to panda_datatypes.h as well
